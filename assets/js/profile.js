/**
 * Handles client-side interactions for the profile page.
 *
 * Responsibilities include:
 * - Profile picture management
 * - Bio expansion and collapse
 * - Post media upload and preview
 * - Post update and deletion
 * - Like and unlike interactions
 * - Comment creation, updating, deletion, and modal handling
 * - Media preview and UI state management
 */
document.addEventListener("DOMContentLoaded", () => {
  // ------------------------------------------------------------
  // Constants
  // ------------------------------------------------------------
  const DEFAULT_PROFILE_IMAGE = "/assets/NoProfileImage.jpg";
  ...
  /**
   * Handles clicks on the profile picture.
   *
   * Opens the file picker when the default image is being used.
   * Otherwise, opens the profile picture management modal.
   *
   * @returns {void}
   */
  const handleProfileImageClick = function () {
    const currentImage = profileImageBtn?.querySelector("img");
    const currentImageSrc = currentImage?.getAttribute("src");

    if (!fileInput) return;

    if (currentImageSrc === DEFAULT_PROFILE_IMAGE) {
      fileInput.click();
      return;
    }
  ...
  profileImageBtn?.addEventListener("click", handleProfileImageClick);

  /**
   * Closes the profile picture edit modal
   * and restores page scrolling.
   *
   * @returns {void}
   */
  const closeProfileImageModal = function () {
    editImage?.classList.remove("active");
    editBg?.classList.remove("active");
    document.body.classList.remove("no-scroll");
  };

  cancelBtn?.addEventListener("click", closeProfileImageModal);
  editBg?.addEventListener("click", closeProfileImageModal);
  ...
  fileInput?.addEventListener("change", (e) => {
    ...
    if (file.size > MAX_PROFILE_IMAGE_SIZE) {
      showFlashMessage("error", "Profile picture must be smaller than 10 MB.");
      input.value = "";
      return;
    }
   ...
   *
   * Sends a DELETE request to the server and updates the profile
   * image in the UI when the request succeeds.
   *
   * @async
  ...
  const removeProfilePicture = async () => {
    ...
    if (!currentImage || currentImageSrc === DEFAULT_PROFILE_IMAGE) {
      return;
    }
  ...
  const initializeBio = () => {
    if (!bioText || !moreText) return;

    const isOverflowing = bioText.scrollHeight > bioText.clientHeight;

    moreText.style.display = isOverflowing ? "inline" : "none";
  };
  initializeBio();
  ...
  moreText?.addEventListener("click", () => {
    ...
    const isExpanded = bioText.classList.toggle("expand");

    moreText.textContent = isExpanded ? "Show less" : "Show more";
  });
   ...
   * and updates the related post upload UI.
   *
   * Validates the selected media type before creating
   * a temporary object URL for the preview.
   *
   * @param {HTMLInputElement} input - File input element.
   * @param {HTMLElement} preview - Preview container.
   * @param {HTMLElement|null} caption - Caption input.
